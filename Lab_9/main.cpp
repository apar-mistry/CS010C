...
void Quicksort_midpoint(int numbers[], int i, int k) {
	...
	while(true) {
		...
		if (left >= right) { //nothing left to sort aka if zero or one elements remain, break and use rightIndex
			break;
		}
		else { //swap
...
void Quicksort_medianOfThree(int numbers[], int i, int k){ 
    if(i >= k) return; //base case : if partition size is 1 or 0 aka nothing left to sort.
    ...
    //partition algorithm
    while(true){
        while(numbers[leftSide] < pivot) leftSide +=1;  //increments left index while numbers[leftSide] < pivot

        while(numbers[rightSide] > pivot) rightSide -= 1; //decrements right index while numbers[rightSide] > pivot

        if(leftSide >= rightSide) break; //nothing left to sort aka if zero or one elements remain, break and use rightIndex
        else{
